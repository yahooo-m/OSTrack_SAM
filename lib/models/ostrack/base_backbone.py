...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z, x):
        ...
        #z, mask_z, ids_restore_z = self.random_masking(z, 0.5)
        #x, mask_x, ids_restore_x = self.random_masking(x, 0.3)
        len_s = x.size()[1]
        if self.add_sep_seg:
        ...
        #s = x[:, -len_s:]
        #z = x[:, :-len_s]
        #mask_tokens = self.mask_token.repeat(x.shape[0], ids_restore_x.shape[1] + 1 - s.shape[1], 1)
        #x_ = torch.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
        #x_ = torch.gather(x_, dim=1, index=ids_restore_x.unsqueeze(-1).repeat(1, 1, x.shape[2]))
        #x = torch.cat([z, x_], dim=1)
        aux_dict = {"attn": None}
